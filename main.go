...
import (
	...
	"github.com/echotools/evr-net-monitor/internal/evr"
...
const (
	...
	defaultUDPPort       = "6792"
	defaultUDPTargets    = "gameserver-chi1.echovrce.com:6792,gameserver-dal1.echovrce.com:6792"
...
func appVersionForFilename() string {
...
func (m *model) addResult(r probeResult) {
	...
	if r.Success {
		...
		s.jitter = appendCapped(s.jitter, s.lastJitterMs, seriesMaxPoints)
		errLine := r.Err
		if !strings.Contains(r.Err, s.target) {
			errLine = fmt.Sprintf("%s: %s", s.target, r.Err)
		}
		m.pushErr(errLine, r.Time)
	}
...
func main() {
	...
	logger.Info("starting evr-net-monitor", zap.String("version", appVersion()))
	...
	if uiEnabled {
		...
		applyLayout = func() {
			grid.Set(
				...
				ui.NewRow(0.36,
					ui.NewCol(0.34, statusTable),
					ui.NewCol(0.24, playersTable),
					ui.NewCol(0.42, infoList),
				),
			)
		}

		header = widgets.NewParagraph()
		header.Title = "EVR Network Monitor " + appVersion()
		...
		infoList.Title = "Insights + Errors"
		infoList.WrapText = false
		infoList.TextStyle = ui.NewStyle(baseText)
		infoList.BorderStyle = ui.NewStyle(palette[4])
		infoList.TitleStyle = ui.NewStyle(palette[4], ui.ColorClear, ui.ModifierBold)
		...
		render = func() {
			...
			helpText := "Scroll info: Up/Down, j/k, mouse wheel"
			header.Text = buildHeaderText(app.wsStatus, cfg, app.matchState) + " | " + helpText
			...
			infoList.Rows = buildInfoRows(app)
			if infoList.SelectedRow >= len(infoList.Rows) {
				infoList.SelectedRow = maxInt(0, len(infoList.Rows)-1)
			}
...
func buildTableRowsStyled(m *model, goodColor, warnColor, badColor, unknownColor ui.Color) ([][]string, map[int]ui.Style) {
	...
	for i := 1; i < len(rows); i++ {
		if i-1 >= len(m.seriesOrder) {
			styles[i] = ui.NewStyle(unknownColor)
			continue
		}
		ps, ok := m.stats[m.seriesOrder[i-1]]
		if !ok {
			styles[i] = ui.NewStyle(unknownColor)
			continue
		}

		if ps.lastTime.IsZero() {
			styles[i] = ui.NewStyle(unknownColor)
			continue
		}

		quality, _ := currentQuality(ps)
		errCount := ps.errors
...
func buildInfoRows(m *model) []string {
	rows := make([]string, 0, len(m.recentErrs)+12)

	insights := buildInsightsRows(m)
	for _, line := range insights {
		rows = append(rows, "[INSIGHT](fg:blue) "+line)
	}

	if len(m.recentErrs) == 0 {
		rows = append(rows, "[ERROR](fg:red,mod:bold) none")
	} else {
		for _, line := range m.recentErrs {
			rows = append(rows, "[ERROR](fg:red,mod:bold) "+line)
		}
	}
